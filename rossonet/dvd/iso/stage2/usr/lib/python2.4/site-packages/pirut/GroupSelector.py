...
class GroupSelector:
    ...
    def __setGroupDescription(self, grp):
        ...
        if cnt == 0 or not self.ayum.isGroupInstalled(grp):
            ...
            # Gah, rhpl fails:
            if cnt != 1:
                txt = _("%d of %d optional packages selected") % (inst, cnt)
            else:
                txt = _("%d of %d optional package selected") % (inst, cnt)
    ...
    def doRefresh(self):
        try:
            ncats = len(self.ayum.comps.categories)
        except yum.Errors.GroupsError:
            ncats = 0
        if ncats == 0:
            self.xml.get_widget("categorySW").hide()
            try:
                grps = map(lambda x: x.groupid, self.ayum.comps.groups)
            except yum.Errors.GroupsError:
                grps = []
            self._populateGroups(grps)
        else:
